...
<head>
    ...
    <style>
        ...
        .kuis-box {
            background-color: #89A98F; color: #4D6464; font-weight: bold;
            padding: 50px; border-radius: 12px; text-align: center;
            transition: 0.3s; cursor: pointer;
            box-shadow: 4px 4px 6px rgba(0, 0, 0, 0.1);
        }
        .topik-text {
            background-color: #BFD8B8; border-radius: 10px;
            padding: 25px; font-size: 16px; margin-top: 10px;
        }
        .home-btn {
            background-color: #BFD8B8; border: none;
            width: 40px; height: 40px; transition: 0.3s;
            cursor: pointer; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
        }
        .kuis-box:hover, .home-btn:hover { transform: scale(1.05); }

        /* CSS TAMBAHAN UNTUK STATUS LOGIN */
        .auth-container {
            position: absolute; top: 20px; right: 20px;
            background-color: rgba(0,0,0,0.2); padding: 10px 15px;
            border-radius: 50px; display: flex; align-items: center;
            gap: 15px; z-index: 10; color: white;
        }
        .auth-link {
...
<body>
    ...
    <!-- [DIPERBAIKI] Status Login/User di Pojok Kanan Atas -->
